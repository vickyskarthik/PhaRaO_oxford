...
void
ImageTF::warpPolar(InputArray _src, OutputArray _dst, Size dsize,
                   Point2f center, double maxRadius, int flags)
{
    ...
    if (!(flags & WARP_INVERSE_MAP))
    {
        ...
        remap(_src, _dst, mapx, mapy, flags & cv::INTER_MAX,
			(flags & WARP_FILL_OUTLIERS) ? cv::BORDER_CONSTANT : cv::BORDER_TRANSPARENT);
    ...
    else
    {
		...
		remap(src, _dst, mapx, mapy, flags & cv::INTER_MAX,	cv::BORDER_CONSTANT);
...
void ImageTF::magSpectrums( InputArray _src, OutputArray _dst)
{
    Mat src = _src.getMat();
...
void ImageTF::divSpectrums( InputArray _srcA, InputArray _srcB, OutputArray _dst, int flags, bool conjB)
{
...
void
ImageTF::fftShift(InputOutputArray _out)
{
    Mat out = _out.getMat();
...
Point2d ImageTF::weightedCentroid(InputArray _src, cv::Point peakLocation, cv::Size weightBoxSize, double* response)
{
    Mat src = _src.getMat();
...
Point2d ImageTF::phaseCorrelateWindow(InputArray _src1, InputArray _src2, InputArray _window,
                                     double* response, array<double, 3> state)
{
    ...
    int win_range = 10;
    int mid = src1.rows/2;
    // Convert coarse estimate (metres) to fine-image pixels for the search
    // ROI centre.  The fine correlation surface has 1:1 pixel-to-bin mapping
    // (1 pixel = RESOL metres), so we convert: pixels = metres / RESOL.
    double pixel_x = round(state.at(0) / RESOL);
    double pixel_y = round(state.at(1) / RESOL);
...
void ImageTF::createHanningWindow(OutputArray _dst, cv::Size winSize, int type)
{
...
ArrayXXf 
ImageTF::highpassfilt(Size size, bool init)
{
    if(init == false) {
    	// High-pass emphasis filter (raised cosine):
    	//   H(u,v) = (1 - cos(π u/N)) * (1 - cos(π v/M))
    	// Suppresses DC and low-frequency content, preserving the
    	// high-frequency edge structure for FMT rotation estimation.
    	VectorXf eta(size.height);
    	VectorXf neta(size.width);

    	for(int i = 0; i < size.height; i++){
    		eta(i) = 1.0f - cos(M_PI * (float)i / (float)(size.height - 1));
    	}
    	for(int i = 0; i < size.width; i++){
    		neta(i) = 1.0f - cos(M_PI * (float)i / (float)(size.width - 1));
    	}

    	ArrayXXf X = eta*neta.transpose();

        preCalc_filter = X;
    }

	return preCalc_filter;
}
